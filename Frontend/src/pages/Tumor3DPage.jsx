...
const ThreeDViewport = ({ volume, location, analysisId, layers, brainOpacity, setBrainOpacity, realisticView }) => {
  ...
  // Cache for original material colors to allow perfect restoration
  ...
  const updateMaterials = () => {
    ...
    model.materials.forEach((mat) => {
      ...
      if (isTumor) {
        // Tumor: Force Red and handle visibility
        mat.setAlphaMode(layers.tumor ? "OPAQUE" : "BLEND");
        pbr.setBaseColorFactor([0.8, 0, 0, layers.tumor ? 1 : 0]);
      } else if (isEdema) {
        // Edema: Force Purple and handle visibility
        mat.setAlphaMode(layers.edema ? "BLEND" : "BLEND");
        pbr.setBaseColorFactor([0.6, 0, 1, layers.edema ? 0.5 : 0]);
      } else {
